...
public class PPP {
	...
	Descriptor[] arrayDes;	// the array of the descriptors
	/*
	...
	private void updateDescriptors(){
		for (int i = 0; i<nDes; i++){
			...
			int maxObs = arrayDes[i].getLength(); // the maximum number of obstructions for one descriptor
			// 0 presents right
			if (type == 0){ 
				for (int j=0; j<maxObs; j++){
					if(rCol+j*2<col-2){

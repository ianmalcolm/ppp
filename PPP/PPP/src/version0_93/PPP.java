...
public class PPP{
	...
	public void mutatePPP(){
		...
		copyDes(arrayDes, tempDes);
		Random generator = new Random();
		short random;
		while(!availability){
			copyDes(tempDes, arrayDes);
			random = (short)generator.nextInt(nDes);
	 ...
	 * 	copy descriptors
	 */
	private void copyDes(Descriptor[] source, Descriptor[] target){
		short n = (short)(source.length);
		for(int i=0; i<n; i++){
			target[i] = new Descriptor(source[i]);
		}
	}
	/*
